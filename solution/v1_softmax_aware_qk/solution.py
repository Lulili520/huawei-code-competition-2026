"""A-only reciprocal Linear preconditioning with multi-order Hessian repair.
...
constants; Weight-derived intermediates terminate inside Weight quantization.
"""
...
def hif4_calibration_and_quantize_weight(
    ...
    gram = _block_gram(activations, precondition) if statistics_valid else None
    crossblock_gram = (
    ...
    tail_crossblock = (
        _tail_crossblock_gram(activations, precondition)
...
def _attention_cross_hessians(
    ...
    """Build block Fisher proxies induced by the calibrated softmax rows."""
    if head_dim % _BLOCK:
        return None
    heads_per_kv = q_num_heads // kv_num_heads
    block_count = head_dim // _BLOCK
    q_hessian = torch.zeros(
        q_num_heads, block_count, _BLOCK, _BLOCK, dtype=torch.float32
    )
    ...
    try:
        for sample in calib_qkv_list:
            ...
            if sequence == 0 or int(k.shape[0]) != sequence:
                return None
            transformed_q = (q / q_smooth).transpose(0, 1)
            transformed_k = (k * kv_smooth).transpose(0, 1)
            repeated_k = transformed_k.repeat_interleave(heads_per_kv, dim=0)
            logits = transformed_q.matmul(repeated_k.transpose(-1, -2))
            logits.mul_(head_dim ** -0.5)
            probability = torch.softmax(logits, dim=-1)
            if not bool(torch.isfinite(probability).all().item()):
                return None

            # K^T(diag(p)-pp^T)K, without materializing the softmax Jacobian.
            mean_k = probability.matmul(repeated_k)
            key_mass = probability.sum(dim=1)
            q_full = torch.einsum(
                "hs,hsd,hse->hde", key_mass, repeated_k, repeated_k
            )
            q_full.sub_(torch.einsum("htd,hte->hde", mean_k, mean_k))
            q_full.div_(float(head_dim))

            # p(1-p) keeps only PSD key-token diagonal blocks for runtime K.
            query_weight = (probability * (1.0 - probability)).sum(dim=-1)
            k_per_q_head = torch.einsum(
                "ht,htd,hte->hde", query_weight, transformed_q, transformed_q
            )
            k_full = k_per_q_head.reshape(
                kv_num_heads, heads_per_kv, head_dim, head_dim
            ).sum(dim=1)
            for block in range(block_count):
                start = block * _BLOCK
                stop = start + _BLOCK
                q_hessian[:, block] += q_full[:, start:stop, start:stop].cpu()
                k_hessian[:, block] += k_full[:, start:stop, start:stop].cpu()
            q_rows += sequence
            k_rows += sequence * heads_per_kv
...
def hif4_calibration_attention(
    ...
    return {
        "q_state": {
            ...
            "selected_alpha": _ATTENTION_ALPHA,
            "rotation_group_size": 0,
            "rotation_reason": "softmax-fisher-no-rotation",
            "importance": q_importance.flatten().cpu(),
            "min_proxy_gain": _ATTENTION_GAIN,
            "proxy_role": "q-softmax-row-shift-invariant-fisher",
        ...
        "k_state": {
            ...
            "min_proxy_gain": _ATTENTION_GAIN,
            "proxy_role": "k-softmax-diagonal-token-fisher",
