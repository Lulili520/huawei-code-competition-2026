# v0_softmax_aware_qk 优化策略

## 基准版本与实现基础

- 对比基准版本：`v0_hessian_repair`。当前统一 300 例结果为 Linear Output MSE `5.876366e-3`、Attention Output MSE `3.786237e-4`、总分 `16383.511407`（见 `solution/v0_hessian_repair/report.md`）。
- 实现基础：`v0_hessian_repair`。完整复制基准版本六接口与 Linear、V、HiF4 编码路径，仅重写 Attention 校准中的 Q/K Hessian 误差模型和相应状态标识。

## 固定输入边界

NVFP4 反量化保持基准版本 `dequantize_nvfp4` 的规则：最后一维按连续 16 值展开，每组 E2M1 值逐项乘输入 E4M3 scale，随后恢复原 shape。实现不估计或替换输入 scale，也不改变数值语义与分组。

## 问题分析与证据分类

### 已验证事实

1. 基准版本 `solution/v0_hessian_repair/solution.py::_attention_cross_hessians` 用对侧张量的未加权二阶矩构造 Q/K Hessian；它没有计算 attention logits、softmax 概率或 softmax 行方向的投影。
2. 基准版本同文件 `hif4_calibration_attention` 以逐通道对侧二阶矩产生 importance，并由 `_attention_hessian_repair` 用二次型比较修复候选。故替换 Hessian 的统计定义会实际改变 Q/K 误差补偿流程，而不是只调整阈值。
...
|---|---|---|---|---|
| 父 Q Hessian 未建模 softmax，评测目标经过 softmax | `Hq=K^T(diag(p)-pp^T)K/d` 且常数 logit 方向为零空间 | 校准逐 query/head 计算稳定 softmax，以中心化后的 K 构造 Q block Hessian | Attention Output MSE 低于基准版本；Linear MSE应不变 | Attention MSE不降，或候选产生非有限值/接口失败 |
| 父 K Hessian 对各 key 行使用同一无条件 Q 协方差 | `J_ii=p_i(1-p_i)` 给出 key 对每个 query 的局部敏感度 | 用 `p(1-p)` 加权 Q 外积，按校准 key 行聚合 K block Hessian | Attention Output MSE 与总分方向改善 | 统一 300 例不优于基准版本即否证当前近似 |
...
- Linear 两接口、V 动态量化、公开六接口签名、NVFP4解析、HiF4 shape/层级格式全部保持基准版本。

## 超参数配置计划

只提供主配置 `solution.py`，不建立 trial。核心变化由精确 softmax Jacobian 定义决定；温度固定为评测公式的 `1/sqrt(head_dim)`。沿用基准版本阻尼与候选接受阈值以隔离误差模型变化。矩阵不可逆、shape不符或非有限时回退普通合法 HiF4量化。
...
预期仅为可证伪方向：Attention MSE 可能下降，Linear MSE 应与基准版本一致。最终由 Runner 的统一 300 例结果将各假设标记为结果支持、结果否证或证据不足。
