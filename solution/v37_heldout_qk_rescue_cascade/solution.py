"""HiF4 V37: frozen-V routing with held-out exact-output Q/K rescue.

The six competition functions remain the complete quantization surface.  Linear is
byte-equivalent to V23.  Attention first preserves V23's measured-cost V route, then
tests the unchanged paired Q/K window expert against the residual left after routed
V repair.  Fit folds determine a cost-first order; two disjoint held-out folds must
both improve exact Softmax output before a shared Q/K mask is published.  Rescue
failure is fail-closed for Q/K and cannot discard the frozen V route.
``hif4_get_diagnostics`` is read-only metadata.
"""
