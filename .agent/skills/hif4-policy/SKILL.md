...
- `实现基础`：声明 `based_on`、`v0_hessian_repair` 或 `scratch`，并写出实际代码来源、复用模块、重写模块及不直接采用对比版本实现的原因。`based_on` 只用于溯源和比较，不形成树关系。
- `固定输入边界`：明确 NVFP4 固定按 E2M1 值乘 E4M3 scale 反量化，每 16 个值共享一个输入 scale；禁止把输入反量化方式作为优化方案。
- `问题分析`：结合基准版本 report、代码和指标，定位一个可验证问题。每项事实给出文件/函数或评测配置/指标定位，说明证据及其影响的是 Linear、Attention 还是格式搜索。
...
- `算法内部超参数计划` 中的所有配置先接受固定 60 例筛选，最多两个配置进入完整 300 例。筛选结果只用于同版本配置晋级，不能形成正式结论。

每个已完成版本提出三个独立结构策略，但它们进入同一个扁平全局池，不构成父子树。local-agent 任务统一由 `.agent/runner.py` 创建和调度。
